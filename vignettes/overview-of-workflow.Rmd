...
We can also limit our results to a specific set of genes by passing a vector of Entrez Gene IDs or Hugo Symbols to the function. This can be useful if, for example, we want to pull all IMPACT gene results for two studies but one of the two uses a much larger panel. we can limit our query to just the genes we want results for:
